...
class WasteDetectionGUI:
    ...
    def deploy_and_patrol(self):
        ...
        self.last_inspect_time = 0 # Prevent infinite inspection loops
        self.yolo_inference_times = []
        ...
        def _task():
            try:
                ...
                # CRITICAL FIX: Point camera STRAIGHT DOWN so X/Y coordinates map accurately to the ground
                import math
                down_pitch = math.radians(-90.0)
                c.simSetCameraPose("0", airsim.Pose(airsim.Vector3r(0, 0, 0), airsim.to_quaternion(down_pitch, 0, 0)), vehicle_name=self.vehicle_name)
                self.log_message("Camera pitched straight down for accurate mapping.")
                
                self.log_message("Starting river patrol... Moving forward very slowly & stably.")
                # Using simpler, robust movement command proven in coastline test
                c.moveByVelocityAsync(1.5, 0.0, 0.0, 3000, vehicle_name=self.vehicle_name)
            except Exception as e:
    ...
    def generate_waste_metrics(self):
        ...
        metrics_text = (
            ...
            f"---------------------------------------------------------------------------\n"
            f"[1] Detection Accuracy & Reliability\n"
            f"{'Overall System Accuracy':<40} | {overall_accuracy:.2f} %\n"
            f"{'Mean Average Precision (mAP@0.5)':<40} | {mAP:.2f} %\n"
            f"{'Precision (False Positive Rejection)':<40} | {precision:.2f} %\n"
            f"{'Recall (True Positive Rate)':<40} | {recall:.2f} %\n"
            f"---------------------------------------------------------------------------\n"
            f"[2] Sensor Resolution & Limits\n"
            f"{'Least Pixels Calculated (Min Size)':<40} | {min_pixels:.1f} px^2\n"
            f"{'Max Detection Range / Altitude':<40} | {det_range:.1f} m\n"
            f"---------------------------------------------------------------------------\n"
            f"[3] Speed & Communication Latency\n"
            f"{'Speed Detection (Inference FPS)':<40} | {fps:.1f} FPS\n"
            f"{'Vision Inference Latency':<40} | {avg_latency:.2f} ms\n"
            f"{'Msg Delivering Time (Telemetry)':<40} | {msg_delivery_time:.2f} ms\n"
            f"---------------------------------------------------------------------------\n"
            f"[4] Autonomous Search & Energy\n"
            f"{'Area Coverage Rate':<40} | {area_coverage:.2f} m^2/s\n"
            f"{'Waste Discovery Rate':<40} | {waste_discovery_rate:.2f} items/min\n"
            f"{'Energy per Detection':<40} | {energy_per_det:.4f} Wh/Item\n"
            f"===========================================================================\n"
        )
        
        print(metrics_text)
        
        # Save to file (Append mode so both scripts save to the same file)
        try:
            with open("simulation_metrics_comparison.txt", "a") as f:
                f.write(metrics_text)
            self.log_message("Metrics successfully appended to simulation_metrics_comparison.txt", color="green")
        except Exception as e:
            self.log_message(f"Failed to save metrics: {e}", color="red")
                    ...
                    if len(img_1d) > 0:
                        ...
                        if self.model and do_yolo:
                            ...
                            if len(confidences) > 0:
                                ...
                                # Show Entropy on UI
                                cv2.putText(img_bgr, f"Entropy: {entropy:.2f}", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
                                
                                # If uncertainty is high, not already inspecting, and cooldown passed
                                if (0.3 < max_conf < 0.75 or entropy > 0.5) and not self.inspecting:
                                    if time.time() - self.last_inspect_time > 10.0:
                                        self.last_inspect_time = time.time()
