...
class SmartVisionDroneGUI:
        ...
        try:
            ...
            try:
                ...
                if 'actor_state_dict' in checkpoint:
                    ...
                    current_state = self.agent.actor.state_dict()
                    ...
                    if 'input_projection.weight' in actor_state:
                        ...
                        new_weight = current_state['input_projection.weight']  # [64, 9]
                        
                        if old_weight.shape != new_weight.shape:
                            print(f"  📐 Adapting state dimension: {old_weight.shape[1]}D → {new_weight.shape[1]}D")
            ...
            dt = 0.15  # 6.67 Hz control loop (was 0.1 = 10 Hz)
            step = 0
            ...
            while self.flight_active and self.running:
                ...
                if step % 100 == 0:
                    ...
                    speed_text = "🐌 SLOW (5 m/s)" if self.flight_mode == "WIND" else "🚀 FAST (12 m/s)"
                ...
                if self.flight_mode == "WIND":
                    ...
                    # NORMAL MODE: Use vision-based obstacle avoidance
                ...
                if not obstacle_avoided:
                    ...
                    if distance_to_goal > 0.1:
                        ...
                        if self.flight_mode == "WIND":
                            ...
                            if distance_to_goal > 30.0:
                                desired_speed = 5.0  # Slow cruise in wind
                            else:
                                desired_speed = max(2.0, distance_to_goal * 0.2)  # Very slow approach
                        else:
                            # Normal mode - speed depends on dynamic mode
                            if self.dynamic_mode:
                    ...
                    safe_airsim_call(self.client.moveByVelocityAsync, float(target_vx), float(target_vy), float(-target_vz), duration=0.15)
    ...
    def smart_landing(self):
        ...
        try:
            ...
            while current_height > safe_descent_height:
                # Check for obstacles below ONLY in NORMAL mode
                if self.flight_mode == "NORMAL":
                    vision_data = self.get_depth_perception()
                    ...
                    if vision_data:
                        ...
                        if center_dist < 10.0:
                            ...
                            if left_clear:
                                ...
                                safe_airsim_call(self.client.moveByVelocityAsync, 0, -6.0, 0, duration=2.0)
                            elif right_clear:
                                print("  → Moving RIGHT to avoid obstacle")
                                safe_airsim_call(self.client.moveByVelocityAsync, 0, 6.0, 0, duration=2.0)
                            else:
                                print("  ← Moving BACK to avoid obstacle")
                                safe_airsim_call(self.client.moveByVelocityAsync, -6.0, 0, 0, duration=2.0)
                ...
                safe_airsim_call(self.client.moveByVelocityAsync, 0, 0, 4.0, duration=0.5)
                time.sleep(0.3)
            ...
            # Phase 2: Navigate to goal position at low altitude (smooth approach)
            goal_pos = np.array([self.goal_x, self.goal_y])
            current_pos = np.array([pos.x_val, pos.y_val])
            ...
            if distance_to_goal > 3.0:
                ...
                # Smooth goal-reaching algorithm (same as main flight)
                move_steps = 0
                max_move_steps = 50
                ...
                while distance_to_goal > 2.0 and move_steps < max_move_steps:
                    ...
                    # Adjust speed based on flight mode and distance
                    if self.flight_mode == "WIND":
                        # Wind mode: slower, more cautious
                        speed = min(3.0, distance_to_goal * 0.3)
                        speed = max(1.0, speed)
                    else:
                        # Normal mode: faster approach
                        speed = min(5.0, distance_to_goal * 0.5)
                        speed = max(2.0, speed)
                    
                    # Move towards goal at safe height
                    safe_airsim_call(self.client.moveByVelocityAsync,
            ...
            print("  🛬 Final landing...")
            safe_airsim_call(self.client.moveByVelocityAsync, 0, 0, 0, 0.5)  # Stop
            time.sleep(0.2)
    ...
    def start_comparison_race(self, window):
        ...
        print("⚠️  Dynamic Mode: DISABLED for fair comparison")
        print("="*80 + "\n")
        
        # Store and disable dynamic mode
        if hasattr(self, 'dynamic_mode'):
            self.original_dynamic_mode = self.dynamic_mode
            self.dynamic_mode = False
            print("⚡ Dynamic Mode: DISABLED for fair comparison")
    ...
    def _fly_drone1_mhappo(self):
        ...
        try:
            while self.comparison_active:
                ...
                # MHA-PPO control: Standard proportional controller
                if distance > 1.0:
                    direction_normalized = direction / distance
                    # Fixed speed for fair comparison (no dynamic mode)
                    desired_speed = 10.0 if distance > 20.0 else max(4.0, distance * 0.4)
                    target_vx = direction_normalized[0] * desired_speed
                    target_vy = direction_normalized[1] * desired_speed
                    target_vz = 0.5  # Maintain altitude
                    
                    safe_airsim_call(self.drone1_client.moveByVelocityAsync, 
                                   float(target_vx), float(target_vy), float(target_vz), 
                                   duration=0.2, vehicle_name="Drone1")
                ...
                if distance < 3.0:
                    ...
                    # Hover briefly then land
                    safe_airsim_call(self.drone1_client.hoverAsync, vehicle_name="Drone1")
                    time.sleep(1.0)
                    print("   Landing Drone 1...")
                    safe_airsim_call(self.drone1_client.landAsync, vehicle_name="Drone1")
        ...
        try:
            while self.comparison_active:
                ...
                if distance > 1.0:
                    ...
                    if distance > 25.0:
                        desired_speed = 11.0  # Slightly faster cruise
                    elif distance > 20.0:
                        desired_speed = 10.5
                    else:
                        desired_speed = max(4.0, distance * 0.45)  # Smoother deceleration
                    ...
                    safe_airsim_call(self.drone2_client.moveByVelocityAsync, 
                                   float(target_vx), float(target_vy), float(target_vz), 
                                   duration=0.2, vehicle_name="Drone2")
                ...
                if distance < 3.0:
                    ...
                    # Hover briefly then land
                    safe_airsim_call(self.drone2_client.hoverAsync, vehicle_name="Drone2")
                    time.sleep(1.0)
                    print("   Landing Drone 2...")
                    safe_airsim_call(self.drone2_client.landAsync, vehicle_name="Drone2")
                    time.sleep(3.0)  # Wait for landing to complete
    ...
    def stop_comparison_race(self):
        ...
        try:
            if self.drone1_client:
                print("   Hovering and landing Drone 1...")
                safe_airsim_call(self.drone1_client.hoverAsync, vehicle_name="Drone1")
                time.sleep(0.5)
                land_result = safe_airsim_call(self.drone1_client.landAsync, vehicle_name="Drone1")
                if land_result:
                    land_result.join()  # Wait for landing to complete
                time.sleep(2.0)  # Wait for drone to settle on ground
                safe_airsim_call(self.drone1_client.armDisarm, False, vehicle_name="Drone1")
                safe_airsim_call(self.drone1_client.enableApiControl, False, vehicle_name="Drone1")
                print("   ✓ Drone 1 landed successfully")
        except Exception as e:
        ...
        try:
            if self.drone2_client:
                print("   Hovering and landing Drone 2...")
                safe_airsim_call(self.drone2_client.hoverAsync, vehicle_name="Drone2")
                time.sleep(0.5)
                land_result = safe_airsim_call(self.drone2_client.landAsync, vehicle_name="Drone2")
                if land_result:
                    land_result.join()  # Wait for landing to complete
                time.sleep(2.0)  # Wait for drone to settle on ground
                safe_airsim_call(self.drone2_client.armDisarm, False, vehicle_name="Drone2")
        ...
        # Restore original dynamic mode setting
        if hasattr(self, 'original_dynamic_mode'):
            self.dynamic_mode = self.original_dynamic_mode
            status = "ENABLED" if self.dynamic_mode else "DISABLED"
            print(f"⚡ Dynamic Mode restored to: {status}")
